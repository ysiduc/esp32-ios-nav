...
public final class GoongPlacesHTTPClient: GoongPlacesClientProtocol {
    ...
    public func autocomplete(
        ...
        return try parseAutocomplete(data)
    }
    ...
    public func placeDetail(placeID: String, sessionToken: String) async throws -> GoongPlace {
        ...
        let (data, response) = try await session.data(from: url)
        try validateHTTPResponse(response)
        return try parsePlaceDetail(data)
    }
    ...
    private func parseAutocomplete(_ data: Data) throws -> [GoongRawPrediction] {
        guard let root = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw GoongSearchError.decodingError("Invalid JSON root")
        }

        // Goong returns {"status":"OK","predictions":[...]}
        // status != OK treated as no-results (e.g. ZERO_RESULTS)
        ...
        return rawPredictions.enumerated().compactMap { index, p -> GoongRawPrediction? in
            ...
            // more_compound provides sub-components
            ...
            let score = p["provider_ranking"] as? Double
    ...
    private func parsePlaceDetail(_ data: Data) throws -> GoongPlace {
