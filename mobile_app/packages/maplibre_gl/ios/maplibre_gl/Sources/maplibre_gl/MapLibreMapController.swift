    ...
    UIGestureRecognizerDelegate
{
    ...
    // Native Liquid Glass Backdrop Host (P5.9 Architecture)
    private var glassBackdropContainer: UIView?
    private var glassSurfaceViews = [String: UIView]()
    ...
    func onMethodCall(methodCall: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "map#updateGlassSurfaces":
            guard let arguments = methodCall.arguments as? [String: Any],
                  let surfaces = arguments["surfaces"] as? [[String: Any]]
            else {
                result(nil)
                return
            }
            updateNativeGlassSurfaces(surfaces: surfaces)
            result(nil)
        case "map#waitForMap":
            if isMapReady {
                result(nil)
...
// MARK: - Native Liquid Glass Backdrop Integration (P5.9)
    ...
    private func updateNativeGlassSurfaces(surfaces: [[String: Any]]) {
        if glassBackdropContainer == nil {
            ...
            glassBackdropContainer = container
        }
        guard let container = glassBackdropContainer else { return }
        ...
        for surf in surfaces {
            ...
            let visible = surf["visible"] as? Bool ?? true
            if !visible {
                continue
            }
            seenIds.insert(id)
        ...
        // 2. Process grouped surfaces (e.g. right-toolbar with unified container)
        ...
        for (groupId, groupList) in groupSurfaces {
            ...
            for s in groupList {
                ...
                let w = CGFloat(s["w"] as? Double ?? 0.0)
                let h = CGFloat(s["h"] as? Double ?? 0.0)
                let radius = CGFloat(s["radius"] as? Double ?? 23.0)
                ...
                if let existing = glassSurfaceViews[id] {
                    ...
                    if MapLibreMapController.isGlassContainerAvailable {
                        let view = MapLibreMapController.createGlassEffectView(
                            variant: variant,
                            cornerRadius: radius,
                            isSelected: isSelected
        ...
        for (id, view) in glassSurfaceViews where !seenIds.contains(id) {
            view.removeFromSuperview()
            glassSurfaceViews.removeValue(forKey: id)
        }
    }
