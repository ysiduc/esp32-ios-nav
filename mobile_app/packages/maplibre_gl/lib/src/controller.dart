...
class MapLibreMapController extends ChangeNotifier {
  ...
  /// iOS PlatformView ID (P5.9)
  int? platformViewId;
  MethodChannel? _channel;
  MethodChannel get _methodChannel =>
      _channel ??= MethodChannel('plugins.flutter.io/maplibre_gl_$platformViewId');

  /// Update native Liquid Glass backdrop surfaces (P5.9)
  Future<void> updateGlassSurfaces(List<Map<String, dynamic>> surfaces) async {
    if (platformViewId == null) return;
    try {
      await _methodChannel.invokeMethod('map#updateGlassSurfaces', {
        'surfaces': surfaces,
      });
    } catch (_) {
      // Ignored if not supported on platform
    }
  }
