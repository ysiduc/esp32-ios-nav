...
class _MapScreenState extends State<MapScreen> {
  ...
  Widget _buildRecentPlaceAvatar(MapPlace p, bool isDark) {
    ...
    final dispLower = p.displayName.toLowerCase();
    ...
    if (nameLower.contains('nhà') || nameLower.contains('home') || catLower.contains('home')) {
      bgColor = isDark ? const Color(0xFF007AFF).withOpacity(0.30) : const Color(0xFFE5F1FF);
      iconColor = const Color(0xFF007AFF);
      iconData = Icons.home_rounded;
    } else if (nameLower.contains('công ty') || nameLower.contains('work') || nameLower.contains('office') || catLower.contains('work')) {
      bgColor = isDark ? const Color(0xFF5E5CE6).withOpacity(0.30) : const Color(0xFFEEEEFF);
      iconColor = const Color(0xFF5E5CE6);
      iconData = Icons.work_rounded;
    } else if (nameLower.contains('ngân hàng') || nameLower.contains('bank') || nameLower.contains('atm') || dispLower.contains('ngân hàng')) {
      bgColor = isDark ? const Color(0xFF34C759).withOpacity(0.30) : const Color(0xFFE8F8EE);
      iconColor = const Color(0xFF34C759);
      iconData = Icons.account_balance_rounded;
    } else if (nameLower.contains('trường') || nameLower.contains('school') || nameLower.contains('university') || dispLower.contains('trường')) {
      bgColor = isDark ? const Color(0xFF32ADE6).withOpacity(0.30) : const Color(0xFFE5F7FF);
      iconColor = const Color(0xFF007AFF);
      iconData = Icons.school_rounded;
    } else if (nameLower.contains('chợ') || nameLower.contains('siêu thị') || nameLower.contains('mart') || nameLower.contains('shop')) {
      ...
      bgColor = isDark ? Colors.white.withOpacity(0.12) : const Color(0xFFE5E5EA);
      iconColor = isDark ? Colors.white70 : const Color(0xFF636366);
  ...
  Future<void> _updateDestinationScreenPoint() async {
    final ctrl = _mapController;
    if (ctrl == null || !_mapReady || _selectedPlace == null) {
      if (_destinationScreenPoint != null && mounted) {
        setState(() => _destinationScreenPoint = null);
      }
      return;
    }
    try {
      ...
      final screenPt = await ctrl.toScreenLocation(pt);
      if (mounted) {
        setState(() {
          _destinationScreenPoint = screenPt;
        });
      }
    } catch (_) {}
  }
  ...
  Future<void> _updateDestinationMarker() async {
    ...
    try {
      ...
      if (_selectedPlace != null) {
        ...
        // Native GPU vector symbol label
        final effectiveName = _getEffectivePlaceName(_selectedPlace!);
        await ctrl.addSymbol(
          ml.SymbolOptions(
            geometry: pt,
            textField: effectiveName,
            textSize: 13.0,
            textColor: _isDarkMap ? '#FFFFFF' : '#1C1C1E',
            textHaloColor: _isDarkMap ? '#1C222E' : '#FFFFFF',
            textHaloWidth: 2.5,
            textOffset: const Offset(0, 1.8),
          ),
        );
      }
      _updateDestinationScreenPoint();
    } catch (e) {
      ...
      body: Stack(
        children: [
          ...
          ml.MapLibreMap(
            ...
            onCameraIdle: () => _updateDestinationScreenPoint(),
        ...
        children: [
          ...
          ListenableBuilder(
            listenable: Listenable.merge([
              ...
              MapNativeGlassController.instance,
            ]),
            builder: (context, _) => Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(
                  'Glass backend: ${AppGlassBackend.currentName(context, null, false, true)}',
                  style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Color(0xFF5E5CE6)),
                ),
                Text(
                  ' • right-toolbar: ${AppGlassBackend.surfaceBackendName(context: context, overlayOwned: false, isGroup: true)}',
                  style: const TextStyle(fontSize: 10, color: Color(0xFF5E5CE6)),
                ),
                Text(
                  ' • bottom-search: ${AppGlassBackend.surfaceBackendName(context: context, overlayOwned: false)}',
                  style: const TextStyle(fontSize: 10, color: Color(0xFF5E5CE6)),
                ),
                Text(
                  ' • drawer: ${AppGlassBackend.surfaceBackendName(context: context, overlayOwned: true)}',
                  style: const TextStyle(fontSize: 10, color: Color(0xFF5E5CE6)),
                ),
                Text(
                  ' • search-sheet: ${AppGlassBackend.surfaceBackendName(context: context, overlayOwned: true)}',
                  style: const TextStyle(fontSize: 10, color: Color(0xFF5E5CE6)),
                ),
                Text(
                  ' • route-sheet: ${AppGlassBackend.surfaceBackendName(context: context, overlayOwned: true)}',
                  style: const TextStyle(fontSize: 10, color: Color(0xFF5E5CE6)),
                ),
              ],
            ),
  ...
  Widget _buildAppleBottomSearchCapsule() {
    return GestureDetector(
      ...
      child: AppGlassSurface(
        surfaceId: 'bottom-search',
        variant: AppGlassVariant.clear,
        height: 50,
        radius: 25,
        border: Border.all(color: Colors.white.withOpacity(0.40), width: 0.5),
                                    ...
                                    children: [
                                      ...
                                      GestureDetector(
                                        ...
                                        child: Container(
                                          ...
                                          child: Row(
                                            children: [
                                              const Icon(Icons.content_paste_rounded, color: Color(0xFF007AFF), size: 20),
                                              const SizedBox(width: 10),
                                              const Expanded(
                                                child: Text(
                                              ...
                                              : _searchService.recentSearches.take(8).map((p) {
                                                  ...
                                                  return Column(
                                                    children: [
                                                      ...
                                                      Container(
                                                        height: 0.5,
                                                        margin: const EdgeInsets.only(left: 62, right: 16),
                                      ...
                                      GestureDetector(
                                        ...
                                        child: Container(
                                          ...
                                          decoration: BoxDecoration(
                                            color: isDark ? const Color(0xFF2C3240) : const Color(0xFF1C1C1E),
            ...
            children: [
                  ...
                  Column(
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: isDark ? const Color(0xFF1C2C4E) : const Color(0xFFE5F0FF),
                          borderRadius: BorderRadius.circular(8),
                        ),
                        child: const Text(
                          'Tùy chọn',
                  ...
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: isDark ? Colors.white.withOpacity(0.25) : Colors.white.withOpacity(0.55),
                          width: 0.8,
                        ),
                      ),
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Icon(Icons.menu_rounded, color: isDark ? Colors.white38 : Colors.black38, size: 18),
                      ],
                    ),
                    Padding(
                      ...
                      child: Align(
                        ...
                        child: Container(width: 2, height: 16, color: isDark ? Colors.white24 : Colors.black12),
                      ),
                    ),
                    Row(
                      children: [
                    ...
                    Row(
                      children: [
                        ...
                        Icon(Icons.mic_none_rounded, color: isDark ? Colors.white38 : Colors.black38, size: 18),
                        const SizedBox(width: 8),
                        Icon(Icons.menu_rounded, color: isDark ? Colors.white38 : Colors.black38, size: 18),
                      ],
                    ),
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            color: isSelected
                                ...
                                : MapOverlayGlassStyle.cardFill(isDark: isDark),
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(
                              color: isSelected
                                  ? const Color(0xFF007AFF)
                                  : (isDark ? Colors.white.withOpacity(0.20) : Colors.white.withOpacity(0.55)),
                              ...
                              Row(
                                children: [
                                  ...
                                  Text(
                                    r.formattedDistance,
                                    style: TextStyle(
                                      color: isSelected ? Colors.white.withOpacity(0.85) : (isDark ? Colors.white60 : Colors.black54),
                                      fontSize: 12,
                                    ),
                                  ),
                                ],
              ...
              if (_isLoadingRoutes) ...[
                ...
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
                  decoration: BoxDecoration(
                    color: isDark ? const Color(0xFF4A1C1C) : const Color(0xFFFFF2F2),
                    borderRadius: BorderRadius.circular(16),
              ...
              Center(
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    CircleAvatar(radius: 3, backgroundColor: isDark ? Colors.white : Colors.black87),
                    const SizedBox(width: 4),
                    CircleAvatar(radius: 3, backgroundColor: isDark ? Colors.white30 : Colors.black26),
                    const SizedBox(width: 4),
                    CircleAvatar(radius: 3, backgroundColor: isDark ? Colors.white30 : Colors.black26),
                  ],
                ),
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isSelected
                ? (isDark ? const Color(0xFF2C3240) : Colors.white.withOpacity(0.85))
                : Colors.transparent,
            borderRadius: BorderRadius.circular(12),
            border: isSelected
                ? Border.all(
                    color: isDark ? Colors.white.withOpacity(0.30) : Colors.white.withOpacity(0.70),
                    width: 0.8,
                  )
                : null,
